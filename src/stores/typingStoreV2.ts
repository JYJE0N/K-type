...
import { isKoreanJamo, filterKoreanJamo } from '@/utils/koreanIME'

interface TypingStoreV2 {
  // State
  isActive: boolean
  isPaused: boolean
  isCompleted: boolean
...
function isDuplicateInput(state: TypingStoreV2, key: string): boolean {
  const now = Date.now()
...
export const useTypingStoreV2 = create<TypingStoreV2>((set, get) => ({
  // Initial state
  isActive: false,
  isPaused: false,
  isCompleted: false,
